...
| 场景 | 核心指标 | 最新耗时 | 性能变化 |
|---|---|---|---|
| 命令命中路由 (`routing_hit_first`) | 核心命令处理前置过滤与路由时长 | `675.56 ns` | **-72.5%** (显著优化) |
| 命令未命中路由 (`routing_miss_all`) | 未匹配消息的过滤与退出时长 | `672.31 ns` | **-73.6%** (显著优化) |
...
| 场景 | 核心指标 | 最新耗时 | 性能变化 |
|---|---|---|---|
| 冷存储读取命中 (`cold_get_hit`) | 从 Redb 冷存储中读取单个 Key 的耗时 | `11.61 µs` | **-20.7%** (性能提升) |
| 高并发热存储吞吐 (`hottable_concurrent_read_write_x100`) | 100路并发读写 DashMap 耗时 | `51.44 µs` | **-22.9%** (性能提升) |
| 冷存储写入 (`cold_insert`) | 写入 Redb 冷存储耗时 | `2.57 ms` | (无变化) |

### 3. 序列化与协议解析

| 场景 | 核心指标 | 最新耗时 | 性能变化 |
|---|---|---|---|
| JSON 序列化 (`json_serialize_message_receive`) | 消息体 JSON 序列化耗时 | `324.62 ns` | **-3.6%** (性能提升) |
| JSON 反序列化 (`json_deserialize_message_receive`) | 消息体 JSON 反序列化耗时 | `1.78 µs` | (无变化) |
| 定长文本获取 (`get_text_single`) | 优化后定长字符串获取耗时 | `25.73 ns` | **-4.6%** (性能提升) |
...
- **`#[handler]` / `register_handlers!` 性能与功能增强**:
    - **零成本过滤**: 宏在 `handle` 方法中加入前置同步检查 (`type_filter` 和 `command_filter`)，只有当命中指令时，才调用 `tokio::spawn` 启动异步 Handler 协程。**此优化避免了为不匹配消息创建昂贵的 `tokio::spawn` 协程，显著降低了路由开销**。
