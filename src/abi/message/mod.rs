...
fn receive_seq_to_send_seq(seq: &SegmentReceive) -> SegmentSend {
    match seq {
        ...
        SegmentReceive::Location(p) => SegmentSend::Location(message_body::location::DataSend {
            lat: p.lat.clone(),
            lon: p.lon.clone(),
        ...
        SegmentReceive::Share(p) => SegmentSend::Share(message_body::share::DataSend {
            url: p.url.clone(),
            title: p.title.clone(),
            content: Some(p.content.clone()),
...
pub fn receive2send_add_prefix(msg: &MessageReceive, prefix: String) -> MessageSend {
    ...
    let mut ret = vec![SegmentSend::Text(message_body::text::Data { text: prefix })];
